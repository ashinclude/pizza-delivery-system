...
// Replace the existing _handleUserAgent method with this one
  Future<void> _handleUserAgent() async {
    ...
    try {
      ...
      if (_messages.length >= 2) {
        ...
        final prevUserMessage =
            _messages.where((m) => m.role == "user").toList();
        if (prevUserMessage.isNotEmpty &&
            prevUserMessage.last.content.toLowerCase().trim() ==
                userMessage.toLowerCase().trim()) {
          _addMessage(
            content:
                "I noticed you sent the same message again. Let me help - I can:\n" +
        ...
        final lastBotMessage = _messages
        ...
        if (lastBotMessage.contains("would you like to") &&
          ...
          _addMessage(
            content:
                "Great! Could you please specify which pizza you'd like to know more about or order?",
            role: "assistant",
            agentType: "user_agent",
          );
          return;
      ...
      if (userMessage.toLowerCase().contains("pizza") ||
          ...
          MENU.any((item) =>
              userMessage.toLowerCase().contains(item.name.toLowerCase()))) {
      ...
      switch (intent) {
        case QueryIntent.orderPizza:
          ...
          if (pizzaMatch != null) {
            ...
            _addMessage(
              content:
                  "I'd be happy to help you order a pizza. Could you please specify which pizza you'd like from our menu?",
              role: "assistant",
              agentType: "user_agent",
            );
            _showMenu();
        ...
        default:
          ...
          if (userMessage.toLowerCase().contains("non") &&
              userMessage.toLowerCase().contains("veg")) {
            _listNonVegPizzas();
      ...
      _addMessage(
        content:
            "I'm here to assist you with our pizza menu and ordering. What would you like to know?",
        role: "assistant",
        agentType: "user_agent",
      );
    }
  ...
  void _listVegPizzas() {
    ...
    final content = "Here are our vegetarian pizzas:\n\n" +
        vegPizzas
            .map((pizza) =>
                "- ${pizza.name}: ₹${pizza.price}\n  Contains: ${pizza.ingredients.join(', ')}")
            .join('\n\n');
    _addMessage(
